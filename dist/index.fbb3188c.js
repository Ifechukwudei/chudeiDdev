...
document.querySelectorAll('a[href^="#"]').forEach((link)=>{
    link.addEventListener("click", (event)=>{
        ...
        const top = target.getBoundingClientRect().top + window.pageYOffset - fixedNavHeight;
...
window.addEventListener("scroll", ()=>{
    if (window.pageYOffset > threshold) header.classList.remove("remove-header");
});
/*const target = document.querySelector(link.getAttribute("href"))
...
function scrollFunction() {
    ...
    if (window.pageYOffset > 300 && footerTop > window.innerHeight) // Show back to top button if window is scrolled down 300px and footer is not in view
    {
    ...
    } else // Hide back to top button if window is not scrolled down 300px or footer is in view
    if (backToTopButton.classList.contains("btnEntrance")) {
...
document.getElementById("contact-form").addEventListener("submit", function(event) {
    ...
    emailjs.send("service_szcqjeo", "template_semk4kc", {
        name: name,
        email: email,
        message: message
...
},{}]},["csuDB","bDbGG"], "bDbGG", "parcelRequire94c2")
